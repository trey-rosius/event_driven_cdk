...
def send_sqs_lambda_resource(
    stack, api, schema, sqs_send_message_role, lambda_execution_role, queue
):
    with open("lambda_fns/send_sqs_message/send_sqs_lambda_resource.py", "r") as file:
        sqs_lambda = file.read()

    sqs_lambda_function = lambda_.CfnFunction(
        stack,
        "send-sqs-event",
        code=lambda_.CfnFunction.CodeProperty(zip_file=sqs_lambda),
        role=sqs_send_message_role.role_arn,
        # the properties below are optional
        architectures=["x86_64"],
        description="lambda-ds",
        environment=lambda_.CfnFunction.EnvironmentProperty(
            variables={"QueueUrl": queue.attr_queue_url}
        ),
    ...
    lambda_send_sqs_message_config_property = (
        appsync.CfnDataSource.LambdaConfigProperty(
            lambda_function_arn=sqs_lambda_function.attr_arn
        )
    )

    lambda_send_sqs_message_ds = appsync.CfnDataSource(
        scope=stack,
        id="lambda-post-order-ds",
        api_id=api.attr_api_id,
        name="lambda_post_order_ds",
        type="AWS_LAMBDA",
    ...
    post_order_resolver = appsync.CfnResolver(
        stack,
        "post-order",
        api_id=api.attr_api_id,
