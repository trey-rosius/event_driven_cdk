...
def get_single_order_lambda_resource(
    stack, api, schema, db_role, lambda_execution_role
):
    with open("lambda_fns/get_single_order/get_single_order.py", "r") as file:
        get_single_order_function = file.read()

    get_single_order_function_resource = lambda_.CfnFunction(
        stack,
        "get",
        code=lambda_.CfnFunction.CodeProperty(zip_file=get_single_order_function),
    ...
    get_order_lambda_datasource = appsync.CfnDataSource(
        scope=stack,
        id="lambda-get-order-ds",
        api_id=api.attr_api_id,
        name="lambda_get_order_ds",
        type="AWS_LAMBDA",
    ...
    ## get order resolver
    get_order_resolver = appsync.CfnResolver(
        stack,
        "get-order",
        api_id=api.attr_api_id,
        field_name="order",
