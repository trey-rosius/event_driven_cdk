...
def update_order_lambda_resource(stack, api, schema, db_role, lambda_execution_role):
    ...
    update_lambda_function = lambda_.CfnFunction(
        stack,
        "update",
        code=lambda_.CfnFunction.CodeProperty(zip_file=update_function),
    ...
    lambda_update_order_ds = appsync.CfnDataSource(
        scope=stack,
        id="lambda-update-order-ds",
        api_id=api.attr_api_id,
        name="lambda_update_order_ds",
        type="AWS_LAMBDA",
        lambda_config=lambda_update_order_config_property,
        service_role_arn=lambda_execution_role.role_arn,
    )

    update_order = appsync.CfnResolver(
        stack,
        "update-order",
        api_id=api.attr_api_id,
